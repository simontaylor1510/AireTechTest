...
- All timestamps passed between the backend and frontend must be timezone-aware.
...
- Appointments can be cancelled, but cancelled appointments cannot be reinstated.
  - I felt this was ambiguous so chose not to address it.
- The client is somewhat concerned that missed appointments waste significant amounts of clinicians' time 
  ...
  - The data is available in the database, but as it is dependent on the clinician, this should be left until after the clinician endpoints are implemented.
- The PANDA currently contains little data about clinicians, but will eventually track data about the specific organisations they currently work for and where they work from.
...
- Improvement of the appointment management API is required to control which parts of the appointment can be modified by which endpoints.
- For the endpoints that return lists of data, it would make sense to implement support for paging to prevent long delays on the client when the data grows.
- In the same vein, it would also make sense to implement filtering and sorting options to allow clients to tailor the data they receive to their needs.